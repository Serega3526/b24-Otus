...
$docId = 48; // идентификатор доктора из инфоблока Доктора
$doctors = \Bitrix\Iblock\Elements\ElementdoctorsTable::getList([
    'select' => [
        'ID',
        'IMYA',
        'PROTSEDURY.ELEMENT' // PROC_IDS_MULTI - множественное поле инфоблока Доктора
    ],
    'filter' => [
        'ID' => $docId,
        'ACTIVE' => 'Y'
...
foreach ($doctors as $doctor) {
    pr($doctor->get('ID'));
    foreach($doctor->getProtsedury()->getAll() as $prItem) {
